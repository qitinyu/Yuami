...
## ⭐Yuami评论系统配置
1. Twikoo 评论系统配置位于 `src/config.ts` 文件,编辑以控制博客的评论系统显示设置。
...
```typescript
  comment: {
    ...
    system: 'twikoo',
    twikoo: {
      envId: 'https://netlifyxxxx.netlify.app/.netlify/functions/twikoo',
      lang: 'zh-CN',
      // Twikoo 前端 CDN 版本配置
      // 修改 git仓库package.json 中的 version 可升级前端版本，如 '1.7.10'
      // cdn 可替换为自定义 CDN 地址
      version: '1.7.10',
...
```typescript
    twikoo: {
      ...
      // Twikoo 前端 CDN 版本配置
      // 详细可看:https://twikoo.js.org/frontend.html处按需复制粘贴即可
...
>[!important]
>这是最重要的配置项。它告诉你的博客前端，去哪里获取和提交评论数据。它的值是你 Twikoo 服务端的部署地址。是 Twikoo 官方提供的一个演示服务地址。你不应该在生产环境中使用它，因为它有访问限制，且数据不保证持久。
...
- 部署会自动触发
>针对*Twikoo*的后端更新方式
- 找到`config.ts`文件，找到twikoo对应区域代码：
```typescript
      // Twikoo 前端 CDN 版本配置
      // 详细可看:https://twikoo.js.org/frontend.html处按需复制粘贴即可
      // 或修改 version 中可升级的前端版本，如 '1.7.10'
      // cdn 可替换为自定义 CDN 地址
      version: '1.7.10',
      cdn: 'https://s4.zstatic.net/npm/twikoo@{version}/dist/twikoo.min.js',
      // 备用 CDN（主 CDN 加载失败时使用）
