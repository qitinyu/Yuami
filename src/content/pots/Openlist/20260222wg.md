...
image: "./cover4.png"
draft: false
sourceLink: "https://yqamm.cc.cd/posts/openlist/20260222wg"
date: 2026-02-23
pubDate: 2026-02-23
# encrypted: false
# password: "0723"
...
> **此时不要关闭这个 CMD 窗口**，[关闭 = 杀掉进程 = 隧道断开。后续设置为 Windows 服务后可解决此问题。](#第九步)
...
## <a id="第九步">第九步：Windows 开机自启</a>

>**问题**：直接在 CMD 中运行 cloudflared，关闭CMD窗口或关机后隧道断开。</br>
...
方案一：创建vbs启动脚本
>这个方法是使用.vbs自启动后在后台作为系统服务运行，会随开机自动启动服务，并自动关闭CMD窗口（依旧实际情况可能需要手动关闭，但是服务不会终止）。
...
' 定义工作目录和命令
' 注意：请确保路径 D:\Openalist\cloudflared 真实存在
strPath = "D:\Openalist\cloudflared"
strCmd = "cloudflared tunnel run openlist"

' 切换到指定目录并执行命令
' 参数 0 表示隐藏窗口（静默）
' 参数 True 表示等待程序结束再退出（关键修改点）
WshShell.Run "cmd /c cd /d """ & strPath & """ && " & strCmd, 0, True
